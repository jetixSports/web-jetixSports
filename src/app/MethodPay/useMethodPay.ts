...
function useMethodPay({_idTournament}:{_idTournament:string}) {
    ...
    const [users, setUsers] = useState<{_id:string,firstName:string,lastName:string}[] | null>()
    ...
    useEffect(() => {
        (async () => {
            ...
            const leaderTeams = teamsRes.data?.filter((item: Teams) => item._idLeader == user._id)
            ...
            const usersRes=await post(process.env.NEXT_PUBLIC_HOST_SERVICE + "/users/getNames/",{_id:ids} )
    ...
    const onSubmit = async ({ _idTeam, playersMembers }: { _idTeam: string, playersMembers: string[] }) => {
        if (!status) return
        setStatus(false)
        const loadingToast = toast.loading('Inscribiendo...');
        const res = await post(process.env.NEXT_PUBLIC_HOST_SERVICE + "/tournaments/inscribe/",{
            _idTeam,playersMembers:Array.isArray(playersMembers)?playersMembers:[playersMembers],_idTournament,_idUser:user?._id
        })
        ...
        toast.success('¡Inscripcion Existosa!');
