...
export default function useSignUp(acceptTerms: boolean) {
  const { post } = useFetch();
  ...
  const onSubmit = async ({
    ...
    if (!acceptTerms)
      return toast.error("No has aceptado los terminos y condiciones");
    if (!status) return;
    setStatus(false);
    const loadingToast = toast.loading("Creando cuenta...");

    const res = await post(
      process.env.NEXT_PUBLIC_HOST_SERVICE + "/auth/signUp",
      {
        firstName: name,
        lastName: lastname,
        username: nameUser,
    ...
    const toastLoad = toast.loading("Redireccionando...");
    setTimeout(() => {
      router.push("/login");
  ...
  return {
    ...
    fields: {
      ...
      passwordSecond: register("passwordSecond", {
        required: "La contraseña es obligatoria",
        validate: (value) => {
          return watch("password") == value
            ? true
            : "la contrasenas no se parecen";
        },
      }),
    },
    errors,
